...
**トークンの保管場所**: GAS の PropertiesService（スクリプトプロパティ）に保管し、フロントエンドには一切返さない。

**自動更新ロジック**: GAS Proxy 内で API 呼び出し前に access_token の有効期限をチェックし、期限切れの場合は refresh_token で自動更新する。
